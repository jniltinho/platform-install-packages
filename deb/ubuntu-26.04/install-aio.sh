#!/bin/bash
# Installs and configures a Kaltura CE 18.20.0 All-In-One on Ubuntu 26.04 from the local repo.
...
# e.g. KALTURA_APT=file:/opt/kaltura-repo/ubuntu-26.04 (not the GitHub download URL)
KALTURA_APT=${KALTURA_APT:-file:/vagrant/deb/ubuntu-26.04/repo}
...
if [ ! -f /etc/apt/sources.list.d/kaltura-local.list ]; then
	...
	# PHP 7.4: the ondrej PPA does not serve 26.04 (resolute); use packages.sury.org
