...
spec = importlib.util.spec_from_file_location('exp9_api', Path(__file__).parent / 'exp9-api/collect.py')
collector = importlib.util.module_from_spec(spec)
spec.loader.exec_module(collector)


class Exp9CleanupTests(unittest.TestCase):
    def check_failure(self, startup):
        ...
        self.assertIn('systemctl stop php83-exp9-api-' + token, calls[-1])
...
class Exp9MatrixTests(unittest.TestCase):
    def test_target_runtime_and_baseline_are_explicit(self):
        self.assertEqual(collector.api_cases(), [
            ('74','original'),('83','original'),('83','exp8'),('83','exp9')])
        self.assertNotIn(('74','exp9'), collector.api_cases())
        self.assertEqual(len(set(collector.api_cases())), 4)


class Exp9AcceptanceTests(unittest.TestCase):
    ...
    def test_candidate_failure_is_not_parity_success(self):
        code, report, _ = self.run_matrix(lambda r,t,c,o,e: (1,o,e) if t=='exp9' else (c,o,e))
        self.assertEqual(code, 1)
        self.assertFalse(report['functional_checks_passed'])
    ...
    def test_unexpected_stdout_is_rejected_and_not_persisted(self):
        ...
        code, report, raw = self.run_matrix(lambda r,t,c,o,e: (c,marker,e) if t=='exp9' else (c,o,e))
        self.assertEqual(code, 1)
        self.assertNotIn(marker, raw)
        row = next(r for r in report['records'] if r['tree']=='exp9')
