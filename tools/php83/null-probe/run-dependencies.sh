...
case "$2" in
 exp4) tree=/home/vagrant/php-exp4-regression/source/server-Rigel-18.20.0 ;;
 exp5) tree=/home/vagrant/php-exp5-regression/source/server-Rigel-18.20.0 ;;
 *) exit 64 ;;
esac
[[ "$3" =~ ^/tmp/kaltura-pdo-mysql\.[a-zA-Z0-9]+$ && -S "$3/mysql.sock" ]] || exit 64
[[ "$(realpath -- "$3")" == "$3" && ! -L "$3/mysql.sock" ]] || exit 64
scripts=/home/vagrant/php-exp5-regression/tests
...
exec sudo systemd-run --quiet --wait --pipe --collect \
 ...
 -p "BindReadOnlyPaths=$tree:/audit/app $scripts:/audit/tests /home/vagrant/php-exp5-regression/null-probe:/audit/probe /home/vagrant/php-mysql-probe/runtime83:/audit/runtime83 $3:/audit/db" \
 /usr/bin/env "PHP83_PROBE_DATADIR=$3/data/" "$runtime" "${ini[@]}" -d log_errors=0 -d allow_url_fopen=0 -d allow_url_include=0 \
 -d date.timezone=UTC /audit/probe/dependencies.php
