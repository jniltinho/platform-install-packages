...
ROOT='/home/vagrant/php-exp4-regression'
HTTP_EXPECTED=[]
for kind in (0,2):
...
def main():
 ...
 previous=remote('python3 /home/vagrant/php-exp3-regression/api/verify-source.py',60)
 if previous.returncode:raise RuntimeError('exp3 reference source drift')
 previous_artifact=json.loads(previous.stdout)
 token=uuid.uuid4().hex
 db={'datadir':'/tmp/kaltura-pdo-mysql.'+token,'unit':'php83-exp4-api-'+token}
 records=[];cleanup={}
 try:
  ...
  for runtime in ['74','83']:
   for tree in ['original','exp3','exp4']:
 ...
 comparisons=[{'runtime':r['runtime'],'tree':r['tree'],'matches_baseline':baseline['returncode']==0 and r['returncode']==0 and r['stdout']==baseline['stdout']} for r in records if r['tree'] in ['exp3','exp4']]
