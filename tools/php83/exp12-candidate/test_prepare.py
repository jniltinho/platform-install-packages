...
class ProposalTests(unittest.TestCase):
 ...
 def test_actual_collision_guard(self):
  def change(d):d['prior']['patches'][0]['path']=p.TARGETS[0]
  with self.assertRaisesRegex(ValueError,'collision'):p.proposal(self.collision_docs())
 def collision_docs(self):
  d=p.documents();d['prior']['patches'][0]['path']=p.TARGETS[0];return d
 def test_supersedes(self):self.reject(lambda d:d['criteria'].update(supersedes='wrong.patch'))
 def test_upstream_policy(self):self.reject(lambda d:d['criteria'].update(upstream_sha256='0'*64))
 def test_exp11_policy(self):self.reject(lambda d:d['criteria'].update(exp11_sha256='0'*64))
 def test_cache_acceptance(self):self.reject(lambda d:d['criteria'].update(cache_acceptance=True))
 def test_prior_preservation_flag(self):self.reject(lambda d:d['criteria'].update(preserved_prior_bytes_after_removing_attribute=False))
 def test_application_acceptance(self):self.reject(lambda d:d['criteria'].update(application_acceptance=True))
 def test_wrong_prior_root(self):self.reject(lambda d:d['prior'].update(upstream_root='wrong'))
 def test_wrong_prior_revision(self):self.reject(lambda d:d['prior'].update(revision='exp10'))
 def test_held_status(self):self.reject(lambda d:d['debug'].update(status='SELECTED'))
