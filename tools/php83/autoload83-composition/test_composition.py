...
class CompositionTests(unittest.TestCase):
    ...
    def test_bool_integer_float_substitutions(self):
        for value in (1,1.0):
            b,h=self.valid_body();next(r for r in b['rows'] if r['case']=='hit')['value']['loaded']=value
            with self.assertRaisesRegex(ValueError,'rows'):m.validate(b,'cli-append','83','candidate',0,'',h)
    def test_noninteger_exits(self):
        for value in (False,0.0,255.0):
            b,h=self.valid_body()
            with self.assertRaisesRegex(ValueError,'integer'):m.validate(b,'cli-append','83','candidate',value,'',h)
