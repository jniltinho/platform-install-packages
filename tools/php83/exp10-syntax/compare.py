...
from scan import compare_reports, load_contract, require, summarize
...
def compare(primary, independent, contract, harness):
    ...
    require(a == b, 'Reports differ beyond per-record duration_ns')
    require(a['schema'] == 1 and a['collection_complete'] is True, 'Incomplete collection')
    ...
    require(set(a['variants']) == {'exp9', 'exp10'}, 'Unexpected variants')
    for name, variant in a['variants'].items():
        require(variant['summary'] == summarize(variant['records']), 'Summary mismatch')
        require(variant['zip_sha256'] == contract['pins'][name], 'Variant ZIP mismatch')
    comparison = compare_reports(a['variants'], contract)
    ...
    old = {row['path']: row for row in a['variants']['exp9']['records']}
    newly_diagnostic = [row for row in a['variants']['exp10']['records']
...
def main():
    ...
    print('23568 independent logical rows equal; 43 repairs compile; 11 rejections remain')
