...
class ContractAndComparisonTests(unittest.TestCase):
    def setUp(self):
        self.contract = {'schema': 1, 'pins': {'exp9': scan.EXP9_PIN, 'exp10': 'e' * 64},
                         'targets': [{'path': f'target-{i}.php', 'exp9_sha256': 'a' * 64,
                                      'exp10_sha256': 'b' * 64} for i in range(43)]}
        old = [{'path': f'target-{i}.php', 'sha256': 'a' * 64, 'exit': 255,
                'diagnostics': 'curly offsets rejected'} for i in range(43)]
        old += [{'path': f'other-{i}.php', 'sha256': 'c' * 64,
                 'exit': 255 if i < 11 else 0,
                 'diagnostics': 'remaining syntax error' if i < 11 else ''}
                for i in range(11784 - 43)]
        new = copy.deepcopy(old)
        for record in new[:43]:
            record.update(sha256='b' * 64, exit=0, diagnostics='')
        self.reports = {'exp9': {'records': old}, 'exp10': {'records': new}}
    def load(self):
    ...
    def test_target_invalid_hash(self):
        self.contract['targets'][0]['exp10_sha256'] = 'unknown'
        with self.assertRaises(RuntimeError): self.load()
    def test_expected_bounded_result_still_has_11_rejections(self):
        result = self.compare()
        self.assertTrue(result['bounded_regression_pass'])
        self.assertEqual(len(result['remaining_rejected_paths']), 11)
        self.assertEqual(result['target_rejections_after'], [])
    def test_target_rejection_is_failure(self):
        self.reports['exp10']['records'][0]['exit'] = 255
        self.assertFalse(self.compare()['bounded_regression_pass'])
    def test_target_timeout_is_incomplete_failure(self):
        self.reports['exp10']['records'][0]['exit'] = 124
        self.assertFalse(self.compare()['bounded_regression_pass'])
    def test_unchanged_diagnostic_drift_fails(self):
        self.reports['exp10']['records'][43]['diagnostics'] += ' changed'
        result = self.compare()
