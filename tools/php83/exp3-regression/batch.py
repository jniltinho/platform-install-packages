...
report={'schema':1,'host':socket.gethostname(),'recorded_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'zip_sha256':sha(archive),'verified_extracted_files':len(names),'harness':{str(p.relative_to(root)):sha(p) for folder in ['tests','exp3-regression'] for p in sorted((root/folder).rglob('*')) if p.is_file()},'rows':[],'application_acceptance':False}
for source in ['original','exp3']:
 for ini in ['standard','minimal']:
  for case in ['environment','legacy-json','zend-json','doc-comment','doc-comment-export','doc-comment-consumer']:
   cmd=['bash',str(root/'exp3-regression/run-one.sh'),source,case,ini]
