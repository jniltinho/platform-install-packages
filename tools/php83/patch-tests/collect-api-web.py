...
def main():
    ...
    names = ['api-permission-schema.sql', 'api-session-schema.sql',
             'run-api-web.sh', 'api-web-router.php', 'api-web-inner.sh', 'api-http-client.py']
    ...
    for runtime in ['74', '83']:
        for tree in ['original', 'candidate']:
            command = ('bash /home/vagrant/php-patch-tests/run-api-web.sh '
                       + runtime + ' ' + tree
    ...
    names = ['api-permission-schema.sql', 'api-session-schema.sql',
             'run-api-web.sh', 'api-web-router.php', 'api-web-inner.sh', 'api-http-client.py', 'collect-api-mysql.py', Path(__file__).name]
    report = dict(scope='Real web/index.php under isolated cli-server HTTP; NOT Apache/FPM or HTTPS acceptance',
