...
def main():
    ...
    names = ['api-bootstrap.php', 'api-permission-schema.sql', 'api-session-schema.sql',
             'run-api-http.sh', 'api-http-router.php', 'api-http-inner.sh', 'api-http-client.py']
    ...
    for runtime in ['74', '83']:
        for tree in ['original', 'candidate']:
            command = ('bash /home/vagrant/php-patch-tests/run-api-http.sh '
                       + runtime + ' ' + tree
    ...
    names = ['api-bootstrap.php', 'api-permission-schema.sql', 'api-session-schema.sql',
             'run-api-http.sh', 'api-http-router.php', 'api-http-inner.sh', 'api-http-client.py', 'collect-api-mysql.py', Path(__file__).name]
    report = dict(scope='Loopback HTTP cli-server adapter + real front controller and JSON; NOT production web/index.php, Apache/FPM or HTTPS acceptance',
