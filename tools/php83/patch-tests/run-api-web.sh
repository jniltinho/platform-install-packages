...
exec sudo systemd-run --quiet --wait --pipe --collect \
 ...
 /usr/bin/env "PHP83_EXPECTED_RUNTIME=$1" "PHP83_PROBE_DATADIR=$3/data/" /bin/bash /audit/tests/api-web-inner.sh "$runtime" "${ini[@]}" -d log_errors=0 -d allow_url_fopen=0 -d allow_url_include=0 \
 -d date.timezone=UTC 
