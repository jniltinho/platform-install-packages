...
def load_contract(path=CONTRACT):
    ...
    require(type(contract.get('schema')) is int and contract.get('schema') == 1, 'Unsupported contract')
    pins = contract.get('pins', {})
    require(set(pins) == {'exp10', 'exp11'} and pins['exp10'] == EXP10_PIN,
            'Wrong paired artifact contract')
    require(isinstance(pins['exp11'], str) and re.fullmatch(r'[0-9a-f]{64}', pins['exp11']),
            'Exp11 pin pending or invalid; do not stage or run')
    require(pins['exp11'] != pins['exp10'], 'Candidate must differ from baseline')
    targets = contract.get('targets', [])
    require(isinstance(targets, list) and len(targets) == 4, 'Expected 4 reviewed targets')
    paths = set()
...
def summarize(records):
    require(all(type(r['exit']) is int and type(r['diagnostics']) is str for r in records), 'Invalid typed status/diagnostics')
...
def runtime_identity():
    ...
    for mounted in ('/audit/tools', '/audit/exp10', '/audit/exp11', '/audit/exp10.zip', '/audit/exp11.zip'):
...
def compare_reports(reports, contract):
    require(set(reports) == {'exp10', 'exp11'}, 'Unexpected variant inventory')
    for name, variant in reports.items():
        for row in variant['records']:
            require(set(row) == {'path','sha256','exit','diagnostics','command','duration_ns'}, 'Unexpected scan row inventory')
            path = row['path']
            require(type(path) is str and path and str(Path(path)) == path and not Path(path).is_absolute() and '..' not in Path(path).parts and Path(path).suffix.lower() in SUFFIXES, 'Invalid PHP path')
            require(type(row['sha256']) is str and re.fullmatch('[0-9a-f]{64}', row['sha256']) is not None, 'Invalid source hash')
            require(type(row['exit']) is int and type(row['diagnostics']) is str, 'Invalid typed status/diagnostics')
            require(type(row['duration_ns']) is int and row['duration_ns'] >= 0, 'Invalid duration')
            require(row['command'] == [PHP]+FLAGS+['/audit/'+name+'/'+path], 'Unexpected compiler command')
    old = {r['path']: r for r in reports['exp10']['records']}
    new = {r['path']: r for r in reports['exp11']['records']}
    require(len(old) == len(reports['exp10']['records']) and
            len(new) == len(reports['exp11']['records']), 'Duplicate scan record')
    ...
    for path, target in targets.items():
        require(old[path]['sha256'] == target['exp10_sha256'] and
                new[path]['sha256'] == target['exp11_sha256'], 'Target identity mismatch')
    ...
    checks = {
        ...
        '4_selected_targets': len(targets) == 4,
        ...
        'observed_exp10_rejections_11': len(old_rejected) == 11,
        'observed_exp11_rejections_7': len(new_rejected) == 7,
    }
    return {'checks': checks, 'bounded_regression_pass': all(checks.values()),
            ...
            'scope': '4-target compiler repair only; remaining rejections are not waived'}
...
def run():
    ...
    for path in old:
        ...
        if a['sha256'] != b['sha256'] or (a['exit'], a['diagnostics']) != (b['exit'], b['diagnostics']):
            changed.append({'path': path, 'source_changed': a['sha256'] != b['sha256'],
                            'exp10_exit': a['exit'], 'exp11_exit': b['exit'],
    ...
    report = {'schema': 1, 'scope': 'paired PHP8.3 -n syntax only, no application runtime acceptance',
              ...
              'candidate_all_files_compile': complete and reports['exp11']['summary']['compiler_rejected'] == 0,
