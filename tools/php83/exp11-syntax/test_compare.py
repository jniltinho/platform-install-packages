...
class ComparisonTests(unittest.TestCase):
    def report(self):
        return {'variants': {'exp10': {'records': [{'duration_ns': 12, 'exit': 255, 'diagnostics': 'failure', 'sha256': 'a'}]},
                             'exp11': {'records': [{'duration_ns': 15, 'exit': 0, 'diagnostics': '', 'sha256': 'b'}]}},
    ...
    def test_ignores_only_per_record_duration(self):
        ...
        b['variants']['exp10']['records'][0]['duration_ns'] += 99
        self.assertEqual(compare.normalized(a), compare.normalized(b))
        self.assertIn('duration_ns', a['variants']['exp10']['records'][0])
    def test_diagnostics_not_ignored(self):
        a = self.report(); b = copy.deepcopy(a)
        b['variants']['exp11']['records'][0]['diagnostics'] = 'Warning'
    ...
    def test_missing_duration_rejected(self):
        a = self.report(); del a['variants']['exp10']['records'][0]['duration_ns']
        with self.assertRaises(KeyError): compare.normalized(a)
    def test_invalid_duration_rejected(self):
        for value in (None, True, -1, '12'):
            a = self.report(); a['variants']['exp10']['records'][0]['duration_ns'] = value
