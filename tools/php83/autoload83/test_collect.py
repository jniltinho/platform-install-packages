...
class CollectorTests(unittest.TestCase):
    ...
    def test_bool_integer_float_substitutions(self):
        for value in (1,1.0):
            b=self.body();next(r for r in b['rows'] if r['case']=='actual-class-hit')['value']=value
            with self.assertRaisesRegex(ValueError,'actual-class-hit'):validate(b,'hp','74','candidate',0,'',{})
    def test_noninteger_exits(self):
        for value in (False,0.0,255.0):
            with self.assertRaisesRegex(ValueError,'integer'):validate(self.body(),'hp','74','candidate',value,'',{})
