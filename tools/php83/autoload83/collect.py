...
def validate_diagnostics(body,case,runtime,variant):
    expected=[]
    if runtime=='74':
        if case=='hp-legacy':expected.append(('setup',8192,'/audit/fixtures/legacy.php',2))
        if variant=='original':
            if family(case)=='hp':expected.append(('load',8192,TARGETS['hp'],17))
            elif family(case)=='core':expected += [('load',8192,TARGETS['core'],n) for n in (188,264)]
            else:expected.append(('cli',8192,TARGETS['cli'],107))
        if family(case)=='cli':expected.append(('cli',8192,'vendor/symfony/vendor/pake/pakeYaml.class.php',54))
    elif case=='cli-tasks-configured':expected.append(('synthetic-config',8192,'alpha/config/kConf.php',7))
    found=[]
    for d in body['diagnostics']:
        require(set(d)=={'phase','severity','file','line','message_sha256'} and re.fullmatch('[0-9a-f]{64}',d['message_sha256']) is not None,'Malformed diagnostic')
        message = ('Function libxml_disable_entity_loader() is deprecated' if d['file']=='alpha/config/kConf.php' else ('Methods with the same name as their class will not be constructors in a future version of PHP; pakeYAMLNode has a deprecated constructor' if d['file'].endswith('/pakeYaml.class.php') else '__autoload() is deprecated, use spl_autoload_register() instead'))
        require(d['message_sha256']==hashlib.sha256(message.encode()).hexdigest(),'Unexpected diagnostic message')
        require(type(d['severity']) is int and type(d['line']) is int,'Diagnostic severity/line must be integer')
        found.append(tuple(d[k] for k in ['phase','severity','file','line']))
    require(found==expected,'Unexpected native diagnostics')

def validate(body,case,runtime,variant,exitcode,stderr,hashes,stdout=''):
    require(type(exitcode) is int,'Exit must be an integer, not bool/float')
    ...
    if case.startswith('fatal-'):
        ...
        require(strict_equal(body['fatal'],{'severity':64,'file':target,'line':{'hp':17,'core':188,'cli':107}[family(case)],'message_sha256':hashlib.sha256(b'__autoload() is no longer supported, use spl_autoload_register() instead').hexdigest()}),'Wrong fatal source/line/message')
        return 'EXPECTED_FATAL'
    validate_diagnostics(body,case,runtime,variant)
    ...
    for key,value in expected(case,runtime,variant).items(): require(key in values and strict_equal(values[key],value),'Expectation mismatch: '+key)
    ...
    if case=='cli-tasks-configured':
        listing=stdout.split('AUTOLOAD_RESULT ')[0]
        require(listing.startswith('available pake tasks:\n'),'Missing actual task listing')
        for task in ['init-project','init-module','propel-build-model']:
            require(re.search(r'^  '+task+r'\s+>',listing,re.M) is not None,'Missing registered task '+task)
    return 'PASS'
...
def main():
    ...
    for runtime,variant,case in matrix:
        ...
        try:
            ...
            body=json.loads(results[0]);status=validate(body,case,runtime,variant,r.returncode,r.stderr,before,r.stdout)
    ...
    listings=[r['stdout'].split('AUTOLOAD_RESULT ')[0] for r in records if r['case']=='cli-tasks-configured']
    listing_equal=len(listings)==3 and all(x==listings[0] for x in listings)
    comparisons.append({'case':'cli-tasks-configured','three_variant_task_listing_identical':listing_equal})
    if not listing_equal:failed+=1
