...
$targets = array('hp'=>'vendor/htmlpurifier/library/HTMLPurifier.autoload.php',
    'core'=>'vendor/symfony/util/sfCore.class.php','cli'=>'vendor/symfony-data/bin/symfony.php');
$family = strpos($case, 'hp') !== false ? 'hp' : (strpos($case, 'core') !== false ? 'core' : 'cli');
$report['target'] = array('path'=>$targets[$family], 'sha256'=>hash_file('sha256', $root.'/'.$targets[$family]));
$phase = 'setup';
$postExit = null;
...
function autoloadProbeFinish() {
    ...
    $GLOBALS['report']['loaded'] = (object) $GLOBALS['report']['loaded'];
...
if (strpos($case, 'fatal-') === 0) {
    ...
    if (strpos($case, 'cli-tasks') !== 0) $postExit = function() {
    ...
    if ($case === 'cli-tasks-configured') {
        $phase = 'synthetic-config';
        require $root.'/alpha/config/kConf.php';
        $cache = kCacheConfFactory::getInstance(kCacheConfFactory::SESSION);
        $cache->store(null, 'local', array('cache_root_path'=>$fixtures.'/project'));
        $cache->store(null, kCacheConfFactory::LOCAL_MEM_CACHE, array('autoload_probe_disabled'=>true));
        $cache->store(null, kCacheConfFactory::REMOTE_MEM_CACHE, array('autoload_probe_disabled'=>true));
        autoloadProbeRow('real-config-value', kConf::get('cache_root_path'));
        autoloadProbeRow('real-session-cache', get_class($cache));
    }
    $argv = array('symfony', strpos($case, 'cli-tasks') === 0 ? '-T' : '-V');
    $argc = count($argv);
    $phase = 'cli';
