...
class Proof(unittest.TestCase):
 ...
 def rejected(self,fn):
  a=self.records();fn(a)
  with self.assertRaises(ValueError):collect.validate(a)
 def test_duplicate_mode(self):self.rejected(lambda a:a.__setitem__(2,a[1]))
 def test_missing_mode(self):self.rejected(lambda a:a.pop())
 def test_unknown_mode(self):self.rejected(lambda a:a[2].update(mode='unknown'))
 def test_reordered_modes(self):self.rejected(lambda a:a.reverse())
 def test_extra_mode(self):self.rejected(lambda a:a.append(a[0]))
 def stderr_change(self,index,value):
  def change(a):
   a[index]['stderr']=value
   a[index]['stderr_sha256']=collect.hashlib.sha256(value.encode()).hexdigest()
  self.rejected(change)
 def test_extra_native_stderr(self):self.stderr_change(1,'unexpected warning\n')
 def test_missing_native_stderr(self):self.stderr_change(0,'')
 def test_extra_fatal_stderr(self):self.stderr_change(3,self.records()[3]['stderr']+'unexpected\n')
 def test_fatal_stdout(self):self.rejected(lambda a:a[3].update(stdout='unexpected'))
 def test_stderr_hash_drift(self):self.rejected(lambda a:a[0].update(stderr_sha256='0'*64))
 def test_explicit_harness_set(self):self.assertEqual(collect.HARNESS_FILES,('build.py','collect.py','probe.php','run.sh','test_ternary.py'))
