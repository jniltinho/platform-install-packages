...
def identity():
 if any(p.is_symlink() for p in analyzer.rglob('*')):raise RuntimeError('Analyzer symlink outside identity policy')
...
with tempfile.TemporaryDirectory(prefix='php83-curly-') as temporary:
 ...
 for path in paths:
  (candidate/path).chmod(0o600)  # Only owned private disposable copies; original remains read-only.
 ...
 if fixed.returncode!=0:raise RuntimeError('PHPCBF failed exit '+str(fixed.returncode)+': '+fixed.stdout+fixed.stderr)
