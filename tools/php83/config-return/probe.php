...
// Preserve mid-iteration state behavior, including historical pointer quirks.
$c = new Zend_Config(['a'=>1,'b'=>2,'c'=>3], true);
$seen=[];
foreach ($c as $key=>$value) {
    $seen[]=[$key,$value];
    if (count($seen)>8) throw new RuntimeException('Unbounded mutation loop');
    if ($key==='a') unset($c->a);
}
expectValue($seen,[['a',1],['b',2],['c',3]],'foreach unset current');
$rows[]=['foreach-unset',$seen,$c->toArray(),count($c)];
$c = new Zend_Config(['a'=>1,'b'=>2,'c'=>3], true);
$c->rewind();$c->next();
foreach (['clone'=>clone $c,'serialize'=>unserialize(serialize($c))] as $label=>$copy) {
    $before=[$copy->key(),$copy->current(),$copy->valid(),count($copy)];
    $rows[]=['mid-iteration',$label,$before,walkConfig($copy)];
}
$object=(object)['marker'=>'synthetic'];$c=new Zend_Config(['object'=>$object],true);
expectValue($c->current()===$object,true,'mixed object identity');
$rows[]=['mixed-object',walkConfig($c)];
$contracts=[];
