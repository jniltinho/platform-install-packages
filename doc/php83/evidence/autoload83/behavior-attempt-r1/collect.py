...
def validate(body,case,runtime,variant,exitcode,stderr,hashes):
    ...
    if case=='cli-tasks': require('post-exit-fixture-phase' not in values,'Task case replaced with version probe')
...
def main():
    ...
    matrix=[(runtime,variant,case) for runtime,variant in [('74','original'),('74','candidate'),('83','candidate')] for case in ['hp','core-simple','core-full','cli-version','cli-version-queue','cli-tasks']]
    ...
    for runtime,variant,case in matrix:
        ...
        except (ValueError,KeyError,TypeError) as e: status='FAIL';error=str(e)
