#!/usr/bin/env python3
"""Strict exp10 bounded runtime reconciliation; pending inputs never become PASS."""
import argparse
import ast
import hashlib
import json
...
RUN = BASE / 'exp10-runtime'
CURRENT = 'de177e6cbaedf3a3207661c2325f466cce37febb73190f3a49d31f24b740c053'
PRIOR = 'cb61e4cd11009bbf9e6e362dfafa5ef2f4af22da817c75fedbd7741a08e44cbc'
ARTIFACT = {'zip_sha256':CURRENT,'verified_extracted_files':15236}
PREVIOUS_ARTIFACT = {'zip_sha256':PRIOR,'verified_extracted_files':15193}
INPUTS = ['api-primary.json','claude-api.json','cli74-primary.json','claude-cli74.json',
          'cli83-primary.json','claude-cli83.json','curly-primary.json','claude-curly.json',
          'runtime-before.json','runtime-after.json','claude-runtime-before.json','claude-runtime-after.json',
          'runtime83-before.json','runtime83-after.json','claude-runtime83-before.json','claude-runtime83-after.json']
...
def local_harness_path(name):
    prefix='/home/vagrant/php-exp10-regression/'
    ...
    if name.startswith('api/'):return REPO/'tools/php83/exp10-api'/name[len('api/'):]
    if name.startswith('exp10-regression/'):return REPO/'tools/php83'/name
...
def api_comparison(a,b,prior_report):
    expected_rows=[('74','original'),('83','original'),('83','exp9'),('83','exp10')]
    expected=[]
    for kind in (0,2):
    ...
    for report in (a,b):
        ...
        verify_harness(report['harness']);file_pin(report,'collector_sha256','tools/php83/exp10-api/collect.py')
        ...
        token=match.group(1);tokens.append(token);unit='php83-exp10-api-'+token
    ...
    expected_diagnostics=next(r['diagnostics'] for r in prior_report['records'] if r['runtime']=='83' and r['tree']=='exp9')
    ...
    return {'logical_rows':4,'independent_rows':rows,'exp9_exp10_diagnostics_exactly_equal':True,'diagnostic_groups':18,'diagnostic_events':503,'distinct_owned_database_tokens':True}
...
def cli_comparison(reports):
    ...
    for runtime in ('74','83'):
        a,b=reports[runtime]
        sources=['original'] if runtime=='74' else ['original','exp9','exp10']
        ...
        for report in (a,b):
            ...
            require(report['zip_sha256']==CURRENT and type(report['verified_extracted_files']) is int and report['verified_extracted_files']==15236,'CLI artifact mismatch')
        ...
        for one,two in zip(a['rows'],b['rows']):
            ...
            if one['source'] in ('exp9','exp10') and one['case']!='environment':
...
def curly_comparison(a,b,prior):
    ...
    file_pin(a,'collector_sha256','tools/php83/exp10-api/collect-curly.py')
    ...
    manifest=load(BASE/'exp10-candidate/manifest.json')
    ...
    return {'classes':3,'logical_cases':68,'independent_equal':True,'exact_prior_candidate83_equal':True,'all59_source_runtime_coverage':False}
...
def runtime_comparison(reports):
    ...
    file_pin(first,'collector_sha256','tools/php83/exp10-api/runtime-identity.py')
    tree=ast.parse((REPO/'tools/php83/exp10-api/runtime-identity.py').read_text())
...
def main():
    ...
    result={'schema':1,'status':'PASS_BOUNDED_RUNTIME_COMPARISON',
        'api':api_comparison(reports['api-primary.json'],reports['claude-api.json'],load(BASE/'exp9-api/codex.json')),
        'cli':cli_comparison({v:(reports[f'cli{v}-primary.json'],reports[f'claude-cli{v}.json']) for v in ('74','83')}),
        ...
        'native83_runtime_identity':native83_runtime_comparison([reports[n] for n in ('runtime83-before.json','runtime83-after.json','claude-runtime83-before.json','claude-runtime83-after.json')],reports['runtime-before.json']),
        ...
        'reference_sha256':{name:digest((BASE/name).read_bytes()) for name in ['exp9-api/codex.json','curly-offsets/behavior-primary.json','exp10-candidate/manifest.json']},
