...
if not j['analyzer_unchanged'] or j['analyzer_before']!=j['analyzer_after'] or j['fix_exit']!=0 or j['scan_after_exit']!=0:
 raise ValueError('Incomplete analyzer execution')
...
with tempfile.TemporaryDirectory(prefix='php83-curly-patch-audit-') as temporary:
 ...
 for index,(path,row) in enumerate(sorted(rows.items()),1):
  ...
  manifest.append({'path':path,'before_sha256':sha(before),'after_sha256':sha(after),'patch':str(patch.relative_to(repo)) if patch.is_absolute() else str(patch),'patch_sha256':sha(diff.stdout),'offset_pairs':changes//2,'changed_bytes':changes})
