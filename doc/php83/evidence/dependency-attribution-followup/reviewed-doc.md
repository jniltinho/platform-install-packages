...
All 4,999 files under `vendor/` contribute to those identity manifests. Reading
...
- Symfony declares **1.0.1**. Its referenced top-level license is not supplied
  by the unrelated nested Propel generator license. `symfony-data` does not
...
- Legacy Google client version, MaxMind Db version/licenses, Akamai license,
...
Independent actual Claude local review/rebuild: **pending**. The reviewer must
check exact source/hash/excerpt agreement and disputed labels, not just totals.
...
Original **5.4**: supplies attribution evidence for later distribution review,
