...
prev=next(r for r in a['records'] if r['runtime']=='83' and r['tree']=='exp8')['diagnostics'];cur=next(r for r in a['records'] if r['runtime']=='83' and r['tree']=='exp9')['diagnostics'];removed=[d for d in prev if d not in cur];added=[d for d in cur if d not in prev]
req(not added and len(removed)==3 and sum(d['count'] for d in removed)==68 and all(d['path'].endswith(('propel/util/PropelPDO.php','lib/db/KalturaStatement.php')) for d in removed))
(base/'exp9-api/comparison.json').write_text(json.dumps({'independent_rows':api,'removed':removed,'new_or_changed_groups':added,'application_acceptance':False},indent=2)+'\n')
ds={v:load(base/f'exp9-runtime/codex-{v}.json') for v in ['74','83']}; comparisons=[];independent=[];zeros=0;fatals=0
for v,d in ds.items():
 other=load(base/f'exp9-runtime/claude-{v}.json');req(len(d['rows'])==len(other['rows'])==(12 if v=='74' else 36))
 ...
 for x,y in zip(d['rows'],other['rows']):
  ...
  if x['source'] in ['exp8','exp9'] and x['case']!='environment':
...
(base/'exp9-runtime/parity.json').write_text(json.dumps({'logical_rows':48,'zero_exits':zeros,'expected_original83_fatals':fatals,'candidate_rows_exit0':12,'previous_artifact_rows_exit0':12,'comparisons':comparisons,'independent':independent,'application_acceptance':False},indent=2)+'\n')
