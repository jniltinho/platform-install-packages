...
a=load(base/'exp8-api/codex.json');b=load(base/'exp8-api/claude.json')
def req(x):
...
prev=next(r for r in a['records'] if r['runtime']=='83' and r['tree']=='exp7')['diagnostics'];cur=next(r for r in a['records'] if r['runtime']=='83' and r['tree']=='exp8')['diagnostics'];removed=[d for d in prev if d not in cur];added=[d for d in cur if d not in prev]
req(not added and len(removed)==6 and sum(d['count'] for d in removed)==132 and all('propel/util/Criteria.php' in d['path'] for d in removed))
(base/'exp8-api/comparison.json').write_text(json.dumps({'independent_rows':api,'removed':removed,'new_or_changed_groups':added,'application_acceptance':False},indent=2)+'\n')
ds={v:load(base/f'exp8-runtime/codex-{v}.json') for v in ['74','83']}; comparisons=[];independent=[];zeros=0;fatals=0
for v,d in ds.items():
 other=load(base/f'exp8-runtime/claude-{v}.json');req(len(d['rows'])==len(other['rows'])==(12 if v=='74' else 36))
 ...
 for x,y in zip(d['rows'],other['rows']):
  ...
  if x['source'] in ['exp7','exp8'] and x['case']!='environment':
...
(base/'exp8-runtime/parity.json').write_text(json.dumps({'logical_rows':48,'zero_exits':zeros,'expected_original83_fatals':fatals,'candidate_rows_exit0':12,'previous_artifact_rows_exit0':12,'comparisons':comparisons,'independent':independent,'application_acceptance':False},indent=2)+'\n')
